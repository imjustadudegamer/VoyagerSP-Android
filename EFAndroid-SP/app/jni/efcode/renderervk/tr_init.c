...
height, which flips it upright (matches retail's bottom-up SHOT). Nearest-sample downscale (no edge
over-read). Resolution-agnostic.
====================
*/
#define SAVETHUMB_DIM 256
static byte		s_saveThumb[ SAVETHUMB_DIM * SAVETHUMB_DIM * 4 ];
static qboolean	s_saveThumbValid = qfalse;

void RB_CaptureSaveThumb( void ) {
	byte	*allsource, *source, *src, *dst;
	size_t	offset = 0;
	int		padlen, x, y, ix, iy, stride;
	int		vw = gls.captureWidth;
	int		vh = gls.captureHeight;

	if ( vw <= 0 || vh <= 0 ) {
		return;
	}

	allsource = RB_ReadPixels( 0, 0, vw, vh, &offset, &padlen, 0 );
	source = allsource + offset;
	stride = 3 * vw + padlen;

	for ( y = 0 ; y < SAVETHUMB_DIM ; y++ ) {
		iy = ( y * vh ) / SAVETHUMB_DIM;
		if ( iy >= vh ) iy = vh - 1;
		for ( x = 0 ; x < SAVETHUMB_DIM ; x++ ) {
			ix = ( x * vw ) / SAVETHUMB_DIM;
			if ( ix >= vw ) ix = vw - 1;
			src = source + iy * stride + ix * 3;
			dst = s_saveThumb + 4 * ( y * SAVETHUMB_DIM + x );
			dst[0] = src[0];
			dst[1] = src[1];
			dst[2] = src[2];
			dst[3] = 255;
	...
	ri.Hunk_FreeTempMemory( allsource );
	s_saveThumbValid = qtrue;
}

// EFSP: arm a save-thumbnail capture for the next end-of-frame (handled in the backend screenshot block).
static void RE_RequestSaveThumb( void ) {
	backEnd.screenshotMask |= SCREENSHOT_SAVETHUMB;
}

// EFSP: copy the latest captured 256x256x4 thumbnail to dst; qfalse if nothing captured yet.
static qboolean RE_GetSaveThumb( byte *dst ) {
	if ( !s_saveThumbValid || dst == NULL ) {
		return qfalse;
	}
	Com_Memcpy( dst, s_saveThumb, sizeof( s_saveThumb ) );
	return qtrue;
}
...
static void R_Register( void )
{
	...
	r_mapOverBrightBits = ri.Cvar_Get( "r_mapOverBrightBits", "0", CVAR_ARCHIVE_ND | CVAR_LATCH ); // retail EF1 default is 0
	...
	ri.Cvar_SetDescription( r_smp, "Run the rendering back-end on a dedicated render thread, overlapping it with the next frame's simulation. Latched; requires vid_restart." );
	...
	r_offsetFactor = ri.Cvar_Get( "r_offsetFactor", "-1", CVAR_CHEAT | CVAR_LATCH );   // EF-correct polygonOffset (HM 2cdcf5e)
	...
	// EF retail configs spell this 'r_ext_compress_textures' (HM 2cdcf5e); register the alias so an
...
void R_Init( void ) {
	...
	// EF/lilium glconfig_t differs from Quake3e's; the engine and renderer are
	// compiled together here, so the Quake3e ABI-size assertion doesn't apply.
	...
	for ( i = 0; i < FUNCTABLE_SIZE; i++ ) {
		...
		tr.noiseTable[i] = R_NoiseGet4f( 0, 0, 0, (double)i );
	...
	{
		// r_smp needs two private back-end data buffers so the render thread can
		// consume frame N while the front end fills frame N+1. Each buffer has its
		// OWN appended polys/polyVerts region (sharing would alias a handed-off
		// frame's polys with the next frame's front-end writes). buffer[1] is
		// allocated only when r_smp is set; buffer[0] is always allocated.
		int b;
...
#ifdef USE_VULKAN
	// EF video-options aniso control gates on this flag; never set under Vulkan -> greyed out (HM 2cdcf5e).
	...
	// r_smp: spawn the render thread LAST, after every init-time Vulkan
	// resource-creation call (pipelines, images, fonts) has run on this thread.
...
static void RE_Shutdown( refShutdownCode_t code ) {
	...
	// r_smp: stop and join the render thread BEFORE tearing anything down, so the
	// front end owns the Vulkan device exclusively for vk_release_resources /
	// vk_shutdown (and before any caller's Hunk_Clear frees backEndDataBuf).
...
//VKPORT 	re.AddLinearLightToScene = RE_AddLinearLightToScene;
	...
	re.RequestSaveThumb = RE_RequestSaveThumb;	// EFSP savegame thumbnail capture
	re.GetSaveThumb = RE_GetSaveThumb;
//VKPORT 	re.SetColorMappings = R_SetColorMappings;

//VKPORT 	re.ThrottleBackend = RE_ThrottleBackend;
//VKPORT 	re.FinishBloom = RE_FinishBloom;
//VKPORT 	re.CanMinimize = RE_CanMinimize;
//VKPORT 	re.GetConfig = RE_GetConfig;
//VKPORT 	re.VertexLighting = RE_VertexLighting;
//VKPORT 	re.SyncRender = RE_SyncRender;
