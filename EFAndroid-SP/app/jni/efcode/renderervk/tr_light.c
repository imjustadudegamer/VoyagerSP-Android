...
void R_SetupEntityLighting( const trRefdef_t *refdef, trRefEntity_t *ent ) {
	...
	if ( ent->e.renderfx & RF_FULLBRIGHT )
	{
		...
		// renderervk keeps the model-space light dir in ent->lightDir (no modelLightDir)
		{
			vec3_t worldUp = { 0, 0, 1 };
			ent->lightDir[0] = DotProduct( worldUp, ent->e.axis[0] );
			ent->lightDir[1] = DotProduct( worldUp, ent->e.axis[1] );
			ent->lightDir[2] = DotProduct( worldUp, ent->e.axis[2] );
		}
		return;
	}
#endif
...
int R_LightForPoint( vec3_t point, vec3_t ambientLight, vec3_t directedLight, vec3_t lightDir )
{
	...
	// SP port: the cgame (CG_GetPlayerLightLevel) can poll lighting during a frame where the
	// world isn't loaded yet (map load/transition) -> tr.world is NULL. Guard it and return
	// safe defaults so we don't deref NULL (was a SIGSEGV on real-mission load) and so the
	// caller doesn't read uninitialised ambient/directed/lightDir.
