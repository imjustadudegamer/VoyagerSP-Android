...
#ifdef ELITEFORCE
	// RF_FORCE_ENT_ALPHA: override the vertex alpha with the entity's shaderRGBA[3]
	// (cloak/fade effects). Blend-enabled stages then fade with the entity alpha.
	if ( backEnd.currentEntity && ( backEnd.currentEntity->e.renderfx & RF_FORCE_ENT_ALPHA ) )
	{
		byte a = backEnd.currentEntity->e.shaderRGBA[3];
		for ( i = 0; i < tess.numVertexes; i++ )
			dest[i].rgba[3] = a;
	}
#endif
...
#ifdef ELITEFORCE
		// RF_FORCE_ENT_ALPHA second half (gl2 tr_shade.c:1060-1073): EF fade
		// effects (transporter materialize, cloak, corpse fade, forcefields)
		// force alpha blending onto stages that are otherwise opaque. The
		// vertex-alpha override alone (R_ComputeColors) is not enough under
		// Vulkan because the blend state is baked into the precompiled
		// pipeline — swap to a blended variant of the same pipeline def.
		// vk_find_pipeline_ext caches by def, so this allocates once per
		// shader stage and is stable afterwards.
		if ( backEnd.currentEntity
			&& ( backEnd.currentEntity->e.renderfx & RF_FORCE_ENT_ALPHA )
			&& backEnd.currentEntity->e.shaderRGBA[3] < 0xFF
			&& !( pStage->stateBits & ( GLS_ATEST_BITS | GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS ) ) )
		{
			Vk_Pipeline_Def def;
			vk_get_pipeline_def( pipeline, &def );
			def.state_bits = ( def.state_bits & ~( GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS | GLS_DEPTHMASK_TRUE ) )
