...
// --- lilium/EF compat: Quake3e engine-core types/defines that lilium's
//     qcommon/renderercommon headers don't provide (renderervk was written
//     against Quake3e's headers). Additive; guarded so they're harmless. ---
#ifndef VK_LILIUM_COMPAT
#define VK_LILIUM_COMPAT
typedef enum { REF_KEEP_CONTEXT, REF_KEEP_WINDOW, REF_DESTROY_WINDOW, REF_UNLOAD_DLL } refShutdownCode_t;
// Quake3e shader-expression tokenizer (ported in vk_compat.c; EF shaders don't use it)
typedef enum {
	TK_GENEGIC = 0, TK_STRING, TK_QUOTED, TK_EQ, TK_NEQ, TK_GT, TK_GTE, TK_LT, TK_LTE,
	TK_MATCH, TK_OR, TK_AND, TK_SCOPE_OPEN, TK_SCOPE_CLOSE, TK_NEWLINE, TK_EOF
} tokenType_t;
...
#define CVAR_DEVELOPER 0
#endif
#ifndef MAX_UINT
#define MAX_UINT 0xFFFFFFFFu
#endif
// math macros/helpers lilium lacks
#ifndef SGN
...
enum {
	...
	SCREENSHOT_SAVETHUMB = 1<<5 // EFSP: capture a 256x256 RGBA savegame thumbnail (into a static buffer, not a file)
};
...
void RB_CaptureSaveThumb( void );	// EFSP: downscale the current frame to a 256x256 RGBA save thumbnail
