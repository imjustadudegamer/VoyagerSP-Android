	...
	if ( glConfig.smpActive ) {
		...
		if ( backEnd.screenshotMask == 0 ) {
			if ( ri.CL_IsMinimized() || backEnd.throttle || r_skipBackEnd->integer ) {
				...
				// Safe without a sync: the render thread only ever processes a
				// buffer handed off via WakeRenderer (after which the front end
				// immediately flips away from it), so the buffer reused here is
				// never the one the render thread is currently reading.
				cmdList->used = 0;
				return;
			}
			...
			// buffer. The handed-off buffer's 'used' must NOT be cleared here;
			// R_InitNextFrame() resets the new front-end buffer.
...
#endif
			// Screenshot / video capture / savegame thumbnail (SCREENSHOT_SAVETHUMB)
			// reads back the just-rendered buffer; run synchronously on this thread
			// with the render thread parked. No buffer flip.
			R_SyncRenderThread();
			cmdList->used = 0;
