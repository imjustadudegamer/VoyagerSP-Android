...
static const void *RB_StretchPic( const void *data ) {
	...
	// EFSP green-border defensive guard (#10): the save-menu text field draws its background box through the
	// WHITE shader in EF's green highlight color. If a degenerate text length yields an absurd quad extent, the
	// scissorless 2D path would smear that green across the whole screen on field blur. Reject clearly-garbage
	// quads (|w|/|h| far beyond any real OR virtual screen) so a bad extent can't paint fullscreen. Legit
	// fullscreen pics (<=640 virtual / <=vidWidth px) are well within +/-16384 and are unaffected.
	if ( cmd->w < -16384.0f || cmd->w > 16384.0f || cmd->h < -16384.0f || cmd->h > 16384.0f ) {
		return (const void *)(cmd + 1);
	}

	// EFSP: normalize a NEGATIVE width/height into a positive quad with swapped tex coords. Retail "flip"
	// draws (the save-menu thumbnail passes a negative height to flip its bottom-up framebuffer capture)
	// produced a degenerate-winding quad here that the Vulkan 2D path rendered INVISIBLY — so the thumbnail
	// never appeared even though the data + draw call were correct. Flipping via tex coords reproduces the
	// intended vertical/horizontal flip and renders right-side up. Positive-dim callers (HUD/menus/cinematics)
	// are untouched (the branches don't fire).
	{
		float x = cmd->x, y = cmd->y, w = cmd->w, h = cmd->h;
		float s1 = cmd->s1, t1 = cmd->t1, s2 = cmd->s2, t2 = cmd->t2;
		if ( h < 0.0f ) { y += h; h = -h; { float t = t1; t1 = t2; t2 = t; } }
		if ( w < 0.0f ) { x += w; w = -w; { float s = s1; s1 = s2; s2 = s; } }
		RB_AddQuadStamp2( x, y, w, h, s1, t1, s2, t2, backEnd.color2D );
	}
	return (const void *)(cmd + 1);
}
...
static const void *RB_SwapBuffers( const void *data ) {
	...
	if ( backEnd.screenshotMask && tr.frameCount > 1 ) {
		...
		if ( backEnd.screenshotMask & SCREENSHOT_SAVETHUMB ) {
			RB_CaptureSaveThumb();	// EFSP: downscale this frame to the 256x256 RGBA save thumbnail
		}
