...
static md3Tag_t *R_GetAnimTag( mdrHeader_t *mod, int framenum, const char *tagName, md3Tag_t * dest) 
{
	...
	{
		// a NEGATIVE frame (e.g. an uninitialised player anim restored by a savegame
		// ReadLevel) slipped past the upper clamp and indexed before the model data
		// -> frame = mod + ofsFrames + framenum*frameSize pointed out of bounds -> SIGSEGV
		// in CG_Player's tag lerp on the first frame after a Load. Clamp the low end too.
		framenum = 0;
	}
