...
void FS_HomeRemove( const char *homePath ) {
	...
	Sys_Remove( FS_BuildOSPath( fs_homepath->string,
			fs_gamedir, homePath ) );
}
...
#ifdef __ANDROID__
		// The retail game data is the one thing the port cannot ship. Tell the
		// user exactly which files are needed and where to put them (shown in
		// an Android dialog via Sys_Dialog before the fatal error).
		Sys_Dialog( DT_ERROR,
			va( "Copy pak0.pk3 - pak3.pk3 from your retail\n"
			    "Elite Force copy into:\n\n"
			    "%s/%s/\n\n"
			    "then restart.", fs_basepath->string, BASEGAME ),
			"Game data missing" );
		// exit immediately — falling into Com_Error would stack a second,
		// useless generic error dialog on top of this one (Sys_Exit is
		// static to sys_main.c; nothing is initialized yet, plain exit is fine)
		exit( 1 );
#endif
