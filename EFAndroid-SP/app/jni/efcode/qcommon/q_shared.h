...
//  #define USE_FLEXIBLE_DISPLAY
  ...
  #define PRODUCT_NAME				"Lilium Voyager SP"
  #define BASEGAME					"baseEF"
  #define CLIENT_WINDOW_TITLE		"Lilium Voyager SP"
  #define CLIENT_WINDOW_MIN_TITLE	"liliumvoysp"
  ...
  // EF1 SP 1:1 boot: the real engine (retail Com_Init tail) does
  //   if (!Com_AddStartupCommands()) Cbuf "cinematic eflogo"
  // i.e. it plays video/eflogo.bik at the END of Com_Init, BEFORE any menu draws.
  // Defining CINEMATICS_LOGO makes lilium's identical boot block do the same, so the
  // EF logo plays first instead of the HM (Holomatch ui.qvm) menu logo. CINEMATICS_INTRO
  // is left undefined to match retail (boot = eflogo only; intro.bik is New-Game).
...
// 4-byte color, accessible as bytes or a packed word (Quake3e renderervk type).
