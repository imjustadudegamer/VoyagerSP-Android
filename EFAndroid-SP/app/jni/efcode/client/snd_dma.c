...
EF1 SP: playing time of a registered sound in milliseconds (0 if invalid). soundLength is the
resampled mono sample count at dma.speed. Used for subtitle scroll timing (CG_S_GETSAMPLELENGTH).
==================
*/
int S_SoundDuration( sfxHandle_t sfxHandle ) {
	if ( !s_soundStarted )                        return 0;
	if ( sfxHandle < 0 || sfxHandle >= s_numSfx )  return 0;
	if ( dma.speed <= 0 )                          return 0;
	return (int)( (double)s_knownSfx[ sfxHandle ].soundLength * 1000.0 / (double)dma.speed );
}
...
#ifdef ELITEFORCE
	// EF has TWO voice channels: CHAN_VOICE (3) is the NON-attenuated dialog/narration channel,
	// CHAN_VOICE_ATTEN (4) is the positional one (game channels.h). Stock ioq3 attenuates every
	// positional sound, so SP cinematic/dialog VO (e.g. the Tuvok tutorial narration, played 3D at
	// the NPC) faded with distance — retail plays CHAN_VOICE at full volume regardless of position.
	// Force full volume on channel 3 to match retail; CHAN_VOICE_ATTEN (4) still attenuates normally.
	if ( entchannel == 3 /*CHAN_VOICE*/ && !fullVolume ) {
		fullVolume = qtrue;
	}
#endif
