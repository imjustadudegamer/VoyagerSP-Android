...
cvar_t	*j_side_axis;
cvar_t	*j_up_axis;
cvar_t	*gp_lookSens;	// gamepad look-speed multiplier (yaw+pitch), surfaced in the joystick settings menu
...
void CL_Disconnect( qboolean showMainMenu ) {
	...
	*clc.downloadTempName = *clc.downloadName = 0;
	...
	if ( ( uivm || SPUI_IsActive() ) && showMainMenu ) {
		if ( SPUI_IsActive() ) SPUI_SetActiveMenu( NULL );
		else VM_Call( uivm, UI_SET_ACTIVE_MENU, UIMENU_NONE );
#ifdef USE_FLEXIBLE_DISPLAY
...
void CL_Vid_Restart_f( void ) {
	...
	// EF1 SP (Route-b): the SP cgame is the dlopen'd libefgame.so, NOT the engine cgvm, so the renderer
	// teardown/reinit below would invalidate every SP model/shader handle while the SP cgame keeps running
	// with stale handles -> black/garbled world. CL_InitCGame() at the tail only re-inits the HM cgvm, never
	// the SP bridge. Refuse the restart while an SP game is live; the changed r_* cvars are archived and take
	// effect on the next SP map load (Video Apply / Controls Reset-Defaults reach here in-game).
	{ extern int SP_IsActive(void);
	  if ( SP_IsActive() && Cvar_VariableIntegerValue("sp_dropHM") ) {
		Com_Printf("vid_restart deferred: settings saved, applied on next map load (SP game active)\n");
		return;
	  } }
...
void CL_CheckTimeout( void ) {
	// EF1 SP (Route b): the game runs in-process with no server packets, so clc.lastPacketTime never
	// advances and this would always "time out" and drop to the menu. SP has no connection to lose.
	{ extern int SP_IsActive(void); if ( SP_IsActive() ) return; }
	//
	// check timeout
	//
...
void CL_Frame ( int msec ) {
	...
	if ( cls.cddialog ) {
		...
		cls.cddialog = qfalse;
		if ( !SPUI_IsActive() ) VM_Call( uivm, UI_SET_ACTIVE_MENU, UIMENU_NEED_CD );  // SP has no CD check
#ifdef USE_FLEXIBLE_DISPLAY
		cls.syncUICursor = qtrue;
#endif
	} else	if ( clc.state == CA_DISCONNECTED && !( Key_GetCatcher( ) & KEYCATCH_UI )
		&& !com_sv_running->integer && ( uivm || SPUI_IsActive() ) ) {
...
void CL_InitRenderer( void ) {
...
#ifdef __ANDROID__
	{
		...
		CL_TouchRegisterShaders( );   // touch UI textures, registered at init (not mid-frame)
	}
#endif
...
#ifdef __ANDROID__
	/* Default to mode 4 (stretch) so the 2D menu fills the screen, and drop
	   CVAR_LATCH so it applies immediately (no vid_restart / stale-config race). */
	cl_flexibleDisplay = Cvar_Get ("cl_flexibleDisplay", "4", CVAR_ARCHIVE);
#else
...
============
*/
// --- stubs for renderervk (Vulkan) refimport additions lilium lacks ---
static void CL_Stub_SetGamma( unsigned char r[256], unsigned char g[256], unsigned char b[256] ) { (void)r;(void)g;(void)b; }
// NOT a stub anymore: renderervk polls this to skip rendering while the app
// is backgrounded. Always returning qfalse defeated the Android lifecycle
// guard — the renderer kept touching a destroyed Vulkan surface on app
// switch/screen-off (intermittent surface-lost ERR_FATAL).
#ifdef __ANDROID__
qboolean Sys_AndroidBackgrounded( void );   // sdl_input.c, set synchronously by the SDL event filter
#endif
static qboolean CL_IsMinimized( void ) {
	cvar_t *m = Cvar_Get( "com_minimized", "0", 0 );
#ifdef __ANDROID__
	if ( Sys_AndroidBackgrounded() )
		return qtrue;
#endif
	return ( m && m->integer ) ? qtrue : qfalse;
}
static void CL_Stub_CvarSetGroup( cvar_t *var, int group ) { (void)var;(void)group; }
static int  CL_Stub_CvarCheckGroup( int group ) { (void)group; return 0; }
static void CL_Stub_CvarResetGroup( int group, qboolean resetModified ) { (void)group;(void)resetModified; }
static void CL_Stub_InitGamma( glconfig_t *config ) { (void)config; }
static void CL_Stub_FreeAll( void ) { }
static void CL_Stub_SetScaling( float factor, int cw, int ch ) { (void)factor;(void)cw;(void)ch; }
static void CL_Stub_SetClipboardBitmap( const byte *bitmap, int size ) { (void)bitmap;(void)size; }
static void CL_Stub_SaveJPG( const char *filename, int q, int w, int h, byte *buf, int pad ) { (void)filename;(void)q;(void)w;(void)h;(void)buf;(void)pad; }
static size_t CL_Stub_SaveJPGToBuffer( byte *b, size_t bs, int q, int w, int h, byte *ib, int pad ) { (void)b;(void)bs;(void)q;(void)w;(void)h;(void)ib;(void)pad; return 0; }
#ifdef USE_VULKAN
// translate Quake3e's string-range Cvar_CheckRange to lilium's float-range one
static void CL_VK_CvarCheckRange( cvar_t *cv, const char *minVal, const char *maxVal, int type ) {
	Cvar_CheckRange( cv, minVal ? atof(minVal) : 0, maxVal ? atof(maxVal) : 0, type == 2 /*CV_INTEGER*/ );
}
#endif
#ifdef USE_VULKAN
...
void CL_InitRef( void ) {
...
#ifdef USE_VULKAN
	ri.Cvar_CheckRange = CL_VK_CvarCheckRange;
#else
	...
	// renderervk (Vulkan) additions
	ri.GLimp_SetGamma = CL_Stub_SetGamma;
	ri.FreeAll = CL_Stub_FreeAll;
	ri.CL_IsMinimized = CL_IsMinimized;
	ri.Cvar_SetGroup = CL_Stub_CvarSetGroup;
	ri.Cvar_CheckGroup = CL_Stub_CvarCheckGroup;
	ri.Cvar_ResetGroup = CL_Stub_CvarResetGroup;
	...
	ri.GLimp_InitGamma = CL_Stub_InitGamma;
	ri.CL_SetScaling = CL_Stub_SetScaling;
	ri.Sys_SetClipboardBitmap = CL_Stub_SetClipboardBitmap;
	ri.CL_SaveJPG = CL_Stub_SaveJPG;
	ri.CL_SaveJPGToBuffer = CL_Stub_SaveJPGToBuffer;
#ifdef USE_VULKAN
...
void CL_Init( void ) {
	...
	gp_lookSens =    Cvar_Get ("gp_lookSens",    "1.0", CVAR_ARCHIVE);
...
//	Cbuf_Execute ();
	...
	{
		extern void SP_RegisterCommands( void );
		SP_RegisterCommands();   // adds the 'spmap <map>' single-player command
	}
}
...
#if defined(STANDALONE) || defined(ELITEFORCE)
	// Elite Force Holomatch is ~25 years old with no key servers; accept any key
	// so the retail ui.qvm never shows the CD-key entry screen.
