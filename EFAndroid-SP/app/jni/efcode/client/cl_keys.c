...
void Field_Paste( field_t *edit ) {
	...
	for ( i = 0 ; i < pasteLen ; i++ ) {
		Field_CharEvent( edit, cbd[i] & 255 );
	}
...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	// tap to CL_Disconnect_f -> main menu. The key is consumed either way (returns here).
	if ( ( key < 128 || key == K_MOUSE1 ) && Key_GetCatcher( ) == 0 && CL_SkipInGameCinematic() ) {
		return;
	}
	...
	if ( key == K_ESCAPE ) {
		...
		if ( !( Key_GetCatcher( ) & KEYCATCH_UI ) ) {
			if ( clc.state == CA_ACTIVE && !clc.demoplaying ) {
				if ( SPUI_IsActive() ) SPUI_SetActiveMenu( "ingame" );
				else VM_Call( uivm, UI_SET_ACTIVE_MENU, UIMENU_INGAME );
			}
...
#ifdef ELITEFORCE
		if ( SPUI_IsActive() ) SPUI_KeyEvent( key, 1 );
		else VM_Call( uivm, UI_KEY_EVENT, key );
#else
...
void CL_CharEvent( int key ) {
	...
	{
		if ( SPUI_IsActive() ) SPUI_KeyEvent( key | K_CHAR_FLAG, 1 );
		else VM_Call( uivm, UI_KEY_EVENT, key | K_CHAR_FLAG, qtrue );
	}
	else if ( Key_GetCatcher( ) & KEYCATCH_MESSAGE ) 
	{
