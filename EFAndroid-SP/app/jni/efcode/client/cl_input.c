...
void CL_MouseEvent( int dx, int dy, int time ) {
	if ( Key_GetCatcher( ) & KEYCATCH_UI ) {
		if ( SPUI_IsActive() ) SPUI_MouseEvent( dx, dy );
		else VM_Call( uivm, UI_MOUSE_EVENT, dx, dy );
#ifndef ELITEFORCE
...
void CL_JoystickMove( usercmd_t *cmd ) {
	...
	// gamepad LOOK sensitivity multiplier (joystick settings menu), scales yaw+pitch. NOTE: there is no
	// "move sensitivity" knob -- gamepad movement saturates to ClampChar(+-127) at ~1.5% stick deflection
	// (j_forward * 32767 >> 127), so scaling forward/side has no perceptible effect (run/no-run).
	float lookScale = gp_lookSens ? gp_lookSens->value : 1.0f;

	float yaw     = j_yaw->value     * cl.joystickAxis[j_yaw_axis->integer]     * lookScale;
	...
	float pitch   = j_pitch->value   * cl.joystickAxis[j_pitch_axis->integer]   * lookScale;
...
void CL_SendCmd( void ) {
	...
	// EF1 SP (Route-b): the bridge runs the game sim IN-PROCESS and forces clc.state=CA_ACTIVE with NO real
	// netchan/server. CL_WritePacket would then build a usercmd packet for a nonexistent localhost server every
	// frame, and because nothing ever acks them the backlog (count = cl.cmdNumber - outPackets[..].p_cmdNumber)
	// grows without bound -> "MAX_PACKET_USERCMDS" spam (cl_input.c:819) + wasted per-frame packet work. The
	// cgame consumes the usercmd that CL_CreateNewCommands() just built ABOVE (so input/look still work); the
	// packet itself goes nowhere. Skip the send entirely while SP is active. (At the menu, clc.state<CA_CONNECTED
	// already returns above, so this only affects in-game SP.) Mirrors the SP_IsActive() gate at cl_main.c:2990.
...
#ifdef ELITEFORCE
	// Button bits must match the SP game's enum (efgame q_shared.h): BUTTON_USE=32 (bit 5),
	// BUTTON_ALT_ATTACK=128 (bit 7). CL_CmdButtons packs in_buttons[i] as (1<<i). The old HM layout
	// (+altattack=bit5, +use=bit6) made the SP game read use-presses as BUTTON_CAREFUL and alt-fire as
	// BUTTON_USE -> terminals/consoles/turbolifts never responded to use, and alt-fire was wrong.
	Cmd_AddCommand ("+use", IN_Button5Down);        // bit 5 = SP BUTTON_USE (32)
	Cmd_AddCommand ("-use", IN_Button5Up);
	Cmd_AddCommand ("+altattack", IN_Button7Down);  // bit 7 = SP BUTTON_ALT_ATTACK (128)
