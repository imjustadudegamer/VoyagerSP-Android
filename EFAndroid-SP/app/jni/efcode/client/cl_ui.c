...
#include "../../sp/sp_ui.h"   // EF1 SP: native UI module (libefui.so) bridge, gated by `sp_ui` cvar
...
static int CL_GetUIScreenPlacement( void ) {
	...
	// EFSP 4:3 (cg_forceAspect, the video-menu Aspect toggle): when 4:3 is requested, pillarbox the menu
	// into a centered 4:3 box. SCR_HOR_CENTER|SCR_VERT_CENTER makes SCR_AdjustTo640 map a screen tap to
	// (sx - screenXBias)/screenXScale — the EXACT inverse of libefui's UI_AdjustFrom640 (uniform scale +
	// uis.bias), since cls.screenXScale/screenXBias use the same 0.5*(vidW - vidH*640/480) math. This keeps
	// menu touch/mouse hit-testing AND the soft-keyboard IME rect (CL_AdjustFromUI) aligned with the
	// pillarboxed draw. Off (default) -> falls through to the existing "16:9 FILL" stretch. The same engine
	// cvar drives the draw side, so the two can't disagree. Overrides the cl_viewmode placement on purpose.
	if ( Cvar_VariableIntegerValue( "cg_forceAspect" ) ) {
		return SCR_VERT_CENTER | SCR_HOR_CENTER;
	}
	...
	if ( cl_viewmode->integer <= 2 ) {
		...
		// modes 4+ = the stretch family ("16:9 FILL" on the EF Android port):
		// the menu stretches to fill like the in-game HUD already does at
		// mode >= 4 (EF's ui.qvm has no ui_new, so the old check left it
		// pillarboxed). Range-based to match cl_scrn.c's CA_ACTIVE block.
		placement = SCR_VERT_STRETCH | SCR_HOR_STRETCH;
	} else {
...
void CL_ShutdownUI( void ) {
	...
	if ( SPUI_IsActive() ) {		// EF1 SP native UI
		SPUI_Shutdown();
		return;
	}
	if ( !uivm ) {
...
void CL_InitUI( void ) {
	...
	// EF1 SP: load the native single-player UI (libefui.so via GetUIAPI) instead of the
	// Holomatch ui.qvm. Defaults ON for the SP app (the real EF1 main menu); SPUI_Load() falls
	// back to the QVM path below if the module fails to load. `seta sp_ui 0` forces the HM menu.
	if ( Cvar_Get("sp_ui", "1", CVAR_ARCHIVE)->integer && SPUI_Load() ) {
		Cvar_SetValue("ui_cdkeychecked2", 1);
		return;					// uivm stays NULL; all UI_* sites route to SPUI_* (SPUI_IsActive)
	}
