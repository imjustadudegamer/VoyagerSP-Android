...
#ifdef __ANDROID__
	/* Android: home == the app's own OBB dir, writable with NO storage permission
	   at any targetSdk. Game data lives in /sdcard/Android/obb/com.voyager.efsp/baseEF/
	   (ANDROID_DATA_ROOT, jni/src/CMakeLists.txt). */
	if( !*homePath )
		Q_strncpyz( homePath, ANDROID_DATA_ROOT, sizeof(homePath) );
	return homePath;
#endif
...
{
#ifdef __ANDROID__
	// Android has no zenity/kdialog/xmessage — use SDL's native message box
	// (renders an Android dialog; also avoids the NULL DESKTOP_SESSION below).
	{
		Uint32 flags;
		switch( type )
		{
		...
		if( type == DT_YES_NO || type == DT_OK_CANCEL )
		{
			// real two-button dialog; on failure return the SAFE answer
			// (NO/CANCEL), never a default "OK"
			...
			buttons[0].text = ( type == DT_YES_NO ) ? "Yes" : "OK";
			buttons[1].flags = SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT;
			buttons[1].buttonid = 0;
			buttons[1].text = ( type == DT_YES_NO ) ? "No" : "Cancel";
			...
			if( SDL_ShowMessageBox( &data, &buttonId ) < 0 || buttonId < 0 )
			{
				Com_Printf( S_COLOR_YELLOW "WARNING: failed to show dialog: %s\n", SDL_GetError() );
				return ( type == DT_YES_NO ) ? DR_NO : DR_CANCEL;
			}
			if( buttonId == 1 )
				return ( type == DT_YES_NO ) ? DR_YES : DR_OK;
			return ( type == DT_YES_NO ) ? DR_NO : DR_CANCEL;
		}
