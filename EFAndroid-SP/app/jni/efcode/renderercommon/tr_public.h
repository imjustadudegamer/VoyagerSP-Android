...
typedef struct {
	...
	// EFSP savegame thumbnails: arm a 256x256 RGBA framebuffer capture (filled at the next end-of-frame,
	// the same safe readback point as screenshots), and copy the latest captured thumbnail into dst
	// (256*256*4 bytes, bottom-up RGBA). GetSaveThumb returns qfalse until a capture has completed.
	void     (*RequestSaveThumb)( void );
	qboolean (*GetSaveThumb)( byte *dst );
...
typedef struct {
...
#ifdef USE_VULKAN
	void	(*Cvar_CheckRange)( cvar_t *cv, const char *minVal, const char *maxVal, int type ); // Quake3e sig
#else
	...
	// --- renderervk (Vulkan) port additions (harmless to gl2) ---
	void (*GLimp_SetGamma)( unsigned char red[256], unsigned char green[256], unsigned char blue[256] );
	void (*FreeAll)( void );
	qboolean (*CL_IsMinimized)( void );
	void (*Cvar_SetGroup)( cvar_t *var, int group );
	int  (*Cvar_CheckGroup)( int group );
	void (*Cvar_ResetGroup)( int group, qboolean resetModified );
