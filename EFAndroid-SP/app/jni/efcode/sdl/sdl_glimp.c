...
#ifdef __ANDROID__
		// The Android GL surface is always the full device screen. Force glConfig
		// to the real drawable size so the viewport fills the whole screen — a
		// smaller r_mode would otherwise render into the bottom-left corner — and
		// so the 2D HUD / on-screen touch buttons stay anchored to the physical
		// device resolution regardless of the selected in-game resolution.
		{
			int dw = 0, dh = 0;
			...
			if ( dw > 0 && dh > 0 )
			{
				...
				ri.Printf( PRINT_ALL, "Android: using full drawable %dx%d\n", dw, dh );
			}
		}
#endif
...
#ifdef __ANDROID__
		/* Lock to landscape on Android. SDLActivity drives the device
		   orientation from this hint, so it must be set before video init;
		   the manifest android:screenOrientation is overridden by SDL. */
...
#ifdef __ANDROID__
	/* Android has no desktop GL: default to requesting an OpenGL ES context. */
...
/*
===========================================================================
SDL Vulkan platform layer for the renderervk (Vulkan) port. The renderer is
GL-free: it creates an SDL_WINDOW_VULKAN window and a VkSurfaceKHR via SDL.
===========================================================================
*/
...
void VKimp_Init( glconfig_t *config )
{
	...
	ri.Printf( PRINT_ALL, "VKimp_Init( )\n" );

	SDL_SetHint( SDL_HINT_ORIENTATIONS, "LandscapeLeft LandscapeRight" );

	if ( !SDL_WasInit( SDL_INIT_VIDEO ) ) {
		if ( SDL_Init( SDL_INIT_VIDEO ) != 0 )
			ri.Error( ERR_FATAL, "VKimp_Init: SDL_Init(VIDEO) failed: %s", SDL_GetError() );
	}

	if ( SDL_Vulkan_LoadLibrary( NULL ) != 0 )
		ri.Error( ERR_FATAL, "VKimp_Init: SDL_Vulkan_LoadLibrary failed: %s", SDL_GetError() );

	if ( SDL_GetDesktopDisplayMode( 0, &dm ) != 0 ) { dm.w = 1280; dm.h = 720; dm.refresh_rate = 60; }
	...
	if ( !SDL_window )
		ri.Error( ERR_FATAL, "VKimp_Init: SDL_CreateWindow(VULKAN) failed: %s", SDL_GetError() );

	SDL_Vulkan_GetDrawableSize( SDL_window, &dw, &dh );
	if ( dw <= 0 || dh <= 0 ) SDL_GetWindowSize( SDL_window, &dw, &dh );

	config->vidWidth = dw;
	config->vidHeight = dh;
	config->windowAspect = (float)dw / (float)dh;
	config->displayFrequency = dm.refresh_rate;
...
void VKimp_Shutdown( qboolean unloadDLL )
{
	...
	if ( unloadDLL ) SDL_Vulkan_UnloadLibrary();
	SDL_QuitSubSystem( SDL_INIT_VIDEO );
}
...
void *VK_GetInstanceProcAddr( VkInstance instance, const char *name )
{
	...
	if ( !gipa ) return NULL;
	return (void *)gipa( instance, name );
}
