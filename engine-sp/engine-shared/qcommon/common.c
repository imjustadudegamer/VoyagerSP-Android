...
#ifdef __ANDROID__
		// One-time Android control/graphics defaults. autoexec.cfg runs every
		// boot AFTER hmconfig.cfg, so putting binds/setas there would clobber
		// anything the user changes in the settings menus. Instead the
		// defaults live in android_defaults.cfg and are applied exactly once
		// per defaults VERSION (bump when the shipped defaults change); from
		// then on hmconfig.cfg (CVAR_ARCHIVE + saved binds) owns them.
#define ANDROID_DEFAULTS_VERSION 4   // v4: re-assert defaults after stamp-persistence fix
		if ( Cvar_VariableIntegerValue( "in_androidDefaults" ) < ANDROID_DEFAULTS_VERSION )
		{
			Cbuf_ExecuteText(EXEC_NOW, "exec android_defaults.cfg\n");
			Cbuf_Execute();
			Cvar_Get( "in_androidDefaults", "0", CVAR_ARCHIVE );
			Cvar_Set( "in_androidDefaults", XSTRING( ANDROID_DEFAULTS_VERSION ) );
		}
#endif
	}
}
...
#ifdef __ANDROID__
	// Com_ExecuteCfg above may have just applied android_defaults.cfg (first
	// boot or version bump). The flag clear above would silently drop the
	// in_androidDefaults stamp — and every value the defaults set — from this
	// session's hmconfig.cfg write, making the defaults re-apply (and clobber
	// user settings) every boot. Re-mark so the stamp persists.
	if ( Cvar_VariableIntegerValue( "in_androidDefaults" ) )
		cvar_modifiedFlags |= CVAR_ARCHIVE;
#endif
