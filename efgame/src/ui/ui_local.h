...
typedef enum{
	...
	MNT_CANNOTSAVE,
	// Append-only (no sp_normaltext.dat entry -> load as "?", labels assigned in C). Used by the
	// video menu's 4:3/16:9 aspect toggle. Inserting mid-list would shift every later MNT_ value.
	MNT_ASPECT_4_3,
	MNT_ASPECT_16_9,
	MNT_MAX
...
typedef enum{
	...
	MBT_VIRTUALSAVE_YES,
	MBT_VIRTUALSAVE_NO,
	// NOTE: append-only. These have no entry in the (pk3-shipped) sp_buttontext.dat, so they load as
	// "?" and their labels are assigned in C (UI_ControlsMouse menu init). Adding them mid-list would
	// shift every later enum and misalign the positionally-parsed button-text data.
	MBT_GAMEPADLOOK,
	MBT_MAX
...
typedef struct {
	...
	float				scalex;
	float				scaley;
	float				bias;		// EFSP 4:3 (cg_forceAspect): wide-screen X centering offset; 0 = full-width stretch (16:9)
	qboolean			firstdraw;
...
extern void			UI_RecalcAspect( void );
...
extern void GamepadLookCallback( void *s, int notification );
