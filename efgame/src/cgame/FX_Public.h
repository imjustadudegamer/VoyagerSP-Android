...
extern FXSpawner *FX_AddSpawner( vec3_t origin, vec3_t angles, vec3_t velocity, vec3_t user, int delay, float variance, float killTime, void *think, localEntity_t *owner = NULL, int radius = DEFAULT_SPAWNER_RADIUS );
extern FXSpawner *FX_AddSpawner( vec3_t origin, vec3_t angles, vec3_t velocity, vec3_t user, int delay, float variance, float killTime, int flags, void *think, localEntity_t *owner = NULL, int radius = DEFAULT_SPAWNER_RADIUS );
// Spawner think functions are void(*)(vec3_t x4). Some call sites pass the function
// directly (MSVC implicitly converted fn->void*; clang won't), others already cast to
// (void*). These forwarders accept the direct-function form and route to the void* impl,
// so neither call style needs editing.
typedef void (*FXSpawnerThink_t)( vec3_t, vec3_t, vec3_t, vec3_t );
static inline FXSpawner *FX_AddSpawner( vec3_t origin, vec3_t angles, vec3_t velocity, vec3_t user, int delay, float variance, float killTime, FXSpawnerThink_t think, localEntity_t *owner = NULL, int radius = DEFAULT_SPAWNER_RADIUS )
{ return FX_AddSpawner( origin, angles, velocity, user, delay, variance, killTime, (void *)think, owner, radius ); }
static inline FXSpawner *FX_AddSpawner( vec3_t origin, vec3_t angles, vec3_t velocity, vec3_t user, int delay, float variance, float killTime, int flags, FXSpawnerThink_t think, localEntity_t *owner = NULL, int radius = DEFAULT_SPAWNER_RADIUS )
{ return FX_AddSpawner( origin, angles, velocity, user, delay, variance, killTime, flags, (void *)think, owner, radius ); }
