...
void CG_AdjustFrom640( float *x, float *y, float *w, float *h ) {
	// EFSP 4:3 (cg_forceAspect, the video-menu Aspect toggle): pillarbox the 2D HUD/UI into the SAME
	// centered 4:3 box the 3D view is clamped to (CG_CalcVrect, cg_view.cpp:192), so the whole frame is
	// 4:3 like retail EF — not a full-width-stretched HUD over a 4:3 world. Use a UNIFORM scale (the Y
	// scale) on X and add the wide-screen centering bias (the formula that ships #if 0'd below). Black
	// side bars are painted in CG_Draw2D. Resolution-agnostic (works on any device screen). When the
	// toggle is off, keep the original full-width anamorphic stretch (16:9).
	if ( cg_forceAspect.integer && cgs.glconfig.vidWidth * 480 > cgs.glconfig.vidHeight * 640 ) {
		...
		*x = *x * cgs.screenYScale + bias;
		*y = *y * cgs.screenYScale;
		*w = *w * cgs.screenYScale;
		*h = *h * cgs.screenYScale;
		return;
	}
	// scale for screen sizes (full-width stretch)
	*x *= cgs.screenXScale;
	*y *= cgs.screenYScale;
	*w *= cgs.screenXScale;
	*h *= cgs.screenYScale;
