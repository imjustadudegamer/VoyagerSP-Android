...
{
	// EF1 SP: per-map cinematic-camera reset. CALLED FROM InitGame (g_main.cpp) at per-map game init,
	// BEFORE the opening target_scriptrunner's camera(ENABLE) runs on the bridge's first ge->RunFrame.
	// It used to be called from CG_INIT (cg_main.cpp CG_GameStateReceived) -- but on a FORWARD transition
	// CG_INIT runs AFTER that opening RunFrame, so this reset wiped the just-enabled opening camera
	// (in_camera back to false, client_camera FOV/origin=0) and the player free-looked + free-walked the
	// bridge cutscene (voy1). Moving the call into InitGame restores retail's order: the per-map module
	// reload zeroes the camera BEFORE SV_SpawnServer's warm-up frames fire the opening script. InitGame
	// runs ONLY on a real map load, never on a vid_restart, so the old `if(!qbVidRestartOccured)` guard
	// (which preserved the live camera across a vid_restart) is no longer needed -- a vid_restart simply
	// never calls this now. The reset is therefore UNCONDITIONAL. DO NOT re-add CGCam_Init() to CG_INIT.
...
void CGCam_Enable( void )
{
	...
	// EFSP: seed the camera pose from the live view BEFORE enabling. ICARUS runs one op per 50ms
	// sim tick, so camera(ENABLE) executes a full tick before the first MOVE/PAN/FOLLOW op sets
	// client_camera.origin/angles. On that bare-ENABLE frame CGCam_Update would copy the stale/zero
	// client_camera.origin -> the view snaps to the world origin (black flash / jump). Retail's
	// lock-step server never draws that lone frame; the port draws ~3x per tick so it shows. Seeding
	// holds the current eye until the next op takes over a tick later (overwritten harmlessly).
	VectorCopy( cg.refdef.vieworg,   client_camera.origin );
	VectorCopy( cg.refdefViewAngles, client_camera.angles );
...
void CGCam_Update( void )
{
	...
	// EFSP defensive: CG_CalcFOVFromX(0) yields fov_x=0 -> a zero-area view -> BLACK screen. A FOV of
	// 0 means the camera is "enabled" but was never positioned (e.g. a stale in_camera over a zeroed
	// client_camera). Clamp to the default so the shot is at worst mis-framed, never black.
	if ( client_camera.FOV  <= 0.0f ) client_camera.FOV  = CAMERA_DEFAULT_FOV;
	if ( client_camera.FOV2 <= 0.0f ) client_camera.FOV2 = CAMERA_DEFAULT_FOV;
	...
	// Build the view matrix from the FINAL camera angles, AFTER the follow/track update and shake.
	// Must be last: the renderer orients the scene from cg.refdef.viewaxis, and CGCam_FollowUpdate (and
	// the pan/track branches) only set cg.refdefViewAngles. The old code ran AnglesToAxis BEFORE the
	// follow lerp overwrote the angles, so the view axis trailed the look-at by one frame's lerp step
	// -> a persistent yaw offset (the camera pointed to the side of the speaker, e.g. 7of9 dialog).
