...
void CG_CalcVrect (void) {
	...
	// Aspect Ratio menu toggle: cg_forceAspect=1 pillarboxes the 3D view to 4:3 on a wider screen
	// (flexible-display Android always reports the native, usually 16:9, vidWidth/Height). The FOV is
	// derived from refdef below, so clamping the width here yields a true 4:3 framing with side bars.
	if ( cg_forceAspect.integer && cg.refdef.width * 3 > cg.refdef.height * 4 )
	{
		cg.refdef.width = ( cg.refdef.height * 4 ) / 3;
		cg.refdef.width &= ~1;
	}
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView ) {
	...
	// EF1 SP (dialog camera FIX D + freeze): EF1 conversations are camera(ENABLE)..shot..camera(DISABLE)
	// ops, ONE shot per spoken LINE, the script blocking on the VO (2-5s) between lines. Each inter-line
	// gap drops in_camera to false. Retail's lock-step 50ms server has no real gap inside a conversation,
	// so the client just holds the last frame; the port's continuous cg.time opens a multi-second gap.
	// LATCH: hold the camera view from the first in_camera frame across the gaps (no first-person flicker),
	// release only when the conversation truly ends (bars done fading AND !player_locked). FREEZE: during a
	// held gap do NOT re-run CGCam_Update -- a stale FOLLOW (re-aims at the ended shot's subject -> bad
	// viewaxis) or TRACK (keeps integrating origin -> drifts out of PVS) would point the view into the void
	// -> transient BLACK 3D that recovers on the next shot. Instead replay the last GOOD camera frame
	// (vieworg/viewaxis/fov snapshotted on the live frame), advancing only the letterbox bar/fade covers.
	// (CGCam_Disable clears neither FOLLOWING/TRACKING nor the subject, hence the stale recompute.)
	{
		static qboolean s_dialogCamLatched = qfalse;
		static vec3_t   s_camFreezeOrg;
		static vec3_t   s_camFreezeAxis[3];
		static float    s_camFreezeFovX = 0.0f, s_camFreezeFovY = 0.0f;
		static qboolean s_camFreezeValid = qfalse;
		extern qboolean player_locked;                 // game-side dialog/cutscene player lock

		if ( in_camera ) {
			s_dialogCamLatched = qtrue;
		} else if ( s_dialogCamLatched ) {
			if ( !( client_camera.info_state & CAMERA_BAR_FADING ) && !player_locked ) {
				s_dialogCamLatched = qfalse;
				s_camFreezeValid   = qfalse;   // conversation over -> drop the freeze
			}
		}
		// Detect camera/latch state changes and publish the cutscene signal to the engine.
		{ static int li=-1, ll=-1, lp=-1;
		  if ( (int)in_camera!=li || (int)s_dialogCamLatched!=ll || (int)player_locked!=lp ) {
		    // publish "a scripted cinematic camera owns the view" to the ENGINE so it can hide the
		    // Android touch overlay during cutscenes (the engine has no view of the cgame's in_camera).
		    // Tracks in_camera OR the dialog-cam latch (covers the inter-line gaps); NOT player_locked
		    // (a locked conversation still shows the HUD + first-person view, where touch-look is wanted).
		    cgi_Cvar_Set( "cg_cameraActive", ( in_camera || s_dialogCamLatched ) ? "1" : "0" );
		    li=in_camera; ll=s_dialogCamLatched; lp=player_locked; } }

		if ( in_camera )
		{
			// LIVE shot: cinematics always render the world (clear any stale teleport/hyperspace
			// RDF flag, else R_AddWorldSurfaces skips the world -> black), let CGCam drive the
			// view, then SNAPSHOT the result as the freeze frame.
			cg.refdef.rdflags &= ~( RDF_NOWORLDMODEL | RDF_HYPERSPACE );
			CGCam_RenderScene();                         // CGCam_Update + CG_CalcVrect
			VectorCopy( cg.refdef.vieworg,     s_camFreezeOrg );
			VectorCopy( cg.refdef.viewaxis[0], s_camFreezeAxis[0] );
			VectorCopy( cg.refdef.viewaxis[1], s_camFreezeAxis[1] );
			VectorCopy( cg.refdef.viewaxis[2], s_camFreezeAxis[2] );
			s_camFreezeFovX  = cg.refdef.fov_x;
			s_camFreezeFovY  = cg.refdef.fov_y;
			s_camFreezeValid = qtrue;
		}
		else if ( s_dialogCamLatched && s_camFreezeValid )
		{
			// HELD GAP: replay the last good frame verbatim. CG_CalcVrect for the viewport only
			// (it never touches vieworg/viewaxis/fov), then restore the frozen eye, then advance
			// ONLY the time-based covers (bar fade so CAMERA_BAR_FADING eventually clears for the
			// latch release; normal fade so a camera(FADE) finishes) -- these write bar_*/fade_* only.
			cg.refdef.rdflags &= ~( RDF_NOWORLDMODEL | RDF_HYPERSPACE );
			CG_CalcVrect();
			VectorCopy( s_camFreezeOrg,     cg.refdef.vieworg );
			VectorCopy( s_camFreezeAxis[0], cg.refdef.viewaxis[0] );
			VectorCopy( s_camFreezeAxis[1], cg.refdef.viewaxis[1] );
			VectorCopy( s_camFreezeAxis[2], cg.refdef.viewaxis[2] );
			cg.refdef.fov_x = s_camFreezeFovX;
			cg.refdef.fov_y = s_camFreezeFovY;
		...
		{
			// Not in a conversation: normal first-person.
			CGCam_UpdateFade();
			inwater = CG_CalcViewValues();
		}
	}   // close the dialog-camera latch+freeze block
	...
	if ( stereoView != STEREO_RIGHT ) {
		...
		cg.oldTime = cg.time;
		// EF1 SP: clamp cg.frametime to a single render-frame span. In retail cg.time is the
		// engine's smoothly-interpolated serverTime, so this delta is always ~one frame. The SP
		// port drives cg.time off the 20Hz bridge cadence (cg.time = g_levelTime-50+g_simResidual),
		// which emits a HUGE delta on the first frame after a map transition (CG_INIT zeroes
		// cg.oldTime while g_levelTime is already large) and spikes on multi-tick catch-up hitches.
		// CGCam_FollowUpdate/TrackUpdate scale by cg.frametime (cg_camera.cpp:675,688,870), so an
		// unclamped spike overshoots LerpAngle -> the cinematic camera SPINS, and the resulting
		// degenerate viewaxis collapses the frustum -> world surfaces cull to BLACK at some angles.
		// Clamp to [0,100]ms: the < 0 floor is stock id behavior; 0 is preserved so a paused frame
		// still freezes; 100ms (2 sim ticks) caps the artifact while never touching a real frame.
		if ( cg.frametime < 0 ) {
			cg.frametime = 0;
