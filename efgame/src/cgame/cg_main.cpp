...
cvarTable_t		cvarTable[] = {
	{ &cg_forceAspect, "cg_forceAspect", "0", CVAR_ARCHIVE },   // 1 = pillarbox the 3D view to 4:3 (video menu Aspect Ratio toggle)
...
//	CG_InitMarkPolys();
	...
	// CGCam_Init() MOVED to InitGame (g_main.cpp): the per-map cinematic-camera reset must run BEFORE the
	// opening cinematic's camera(ENABLE) (which fires on the bridge's first ge->RunFrame), not here at
	// CG_INIT -- on a FORWARD transition CG_INIT runs AFTER that RunFrame and wiped the just-enabled camera
	// -> the player could free-look + free-walk the bridge cutscene (voy1). DO NOT re-add CGCam_Init() here.
	FX_Init();
...
//	cgs.media.charsetShader = cgi_R_RegisterShaderNoMip( "gfx/2d/bigchars" );
	...
	// EF1 SP: seed cg.refdef.width/height so the VERY FIRST rendered frame has a valid viewport. If the
	// opening shot is a camera (it is on New Game now that g_levelTime starts at 1000 -> the cinematic
	// camera is active on frame 1), CGCam_Update -> CG_CalcFOVFromX reads refdef.width BEFORE
	// CGCam_RenderScene's CG_CalcVrect has run it; an unseeded width=0 -> fov_y=0 -> a 1-frame BLACK.
	{ extern void CG_CalcVrect( void ); CG_CalcVrect(); }
...
void CG_Shutdown( void )
{
	...
	// ANDROID SP-BRIDGE FIX: retail RELOADS the cgame DLL per map, so CG_PreInit (which zeroes iCGResetCount)
	// runs every map and CG_GameStateReceived does a full fresh reset (memset cg, reset client_camera). This
	// port REUSES the .so across maps (no dlclose), so iCGResetCount accumulates and every map after the first
	// is misdetected as a vid_restart -> cg/camera/animation state is NOT reset and LEAKS across maps. Most
	// visibly: a scripted camera left enabled in Virtual Voyager keeps `in_camera` true into the next mission,
	// so the game freezes the player (g_active.cpp reads in_camera). The bridge calls CG_SHUTDOWN before every
	// map re-init, so reset the accumulators here to make the next map a true fresh start (== retail's reload).
	extern int  iCGResetCount;  iCGResetCount = 0;
	extern bool in_camera;      in_camera     = false;
}
