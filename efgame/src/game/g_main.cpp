...
void InitGame(  const char *mapname, const char *spawntarget, int checkSum, const char *entities, int levelTime, int randomSeed, int globalTime, SavedGameJustLoaded_e eSavedGameJustLoaded, qboolean qbLoadTransition )
{
	...
	// EF1 SP 1:1 (forward-transition opening cinematic lock): zero the cinematic camera HERE, at per-map
	// game init, BEFORE the opening target_scriptrunner's camera(ENABLE) runs. That ENABLE fires on the
	...
	// g_target.cpp level.time<1000 warm-up gate and fires on RunFrame #1) -> CG_INIT then wiped the
	// just-enabled camera -> in_camera=0 -> the player could free-look AND free-walk the bridge cutscene
	// (voy1). Resetting here fixes EVERY map's forward transition. (vid_restart does NOT call InitGame, so
	// the live camera is correctly preserved across it -- the case the old qbVidRestartOccured guard
	// handled.)
	{ extern void CGCam_Init( void ); CGCam_Init(); }
