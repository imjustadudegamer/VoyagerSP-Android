...
int	CNode::Save( int numNodes, fileHandle_t file )
{
	...
	unsigned int header = NODE_HEADER_ID;   // 4-byte FourCC ('NODE'); unsigned long was 8 bytes on LP64
...
{
	unsigned int header;   // 4-byte FourCC on disk; unsigned long read 8 bytes on LP64 (arm64) -> node header mismatch
...
bool CNavigator::Load( const char *filename, int checksum )
{
	...
	//Check the header id
	int navID = GetInt( file );   // .nav header is a 4-byte FourCC ('JNV2'); GetLong read 8 bytes on LP64 (arm64) -> desync
...
bool CNavigator::Save( const char *filename, int checksum )
{
	...
	unsigned int id = NAV_HEADER_ID;   // 4-byte FourCC on disk; unsigned long would write 8 bytes on LP64
