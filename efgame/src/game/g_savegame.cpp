...
*/
	...
	case F_BOOLPTR:
		// EF1 SP arm64 (LP64): test the WHOLE pointer, not just its low 32 bits. The field is an 8-byte
		// pointer on LP64; `!!(*(int*)pv)` only looked at the low word, so a non-NULL pointer whose low 32
		// bits happened to be 0 would serialize as qfalse while the per-entity write condition (which tests
		// the full 8-byte field, e.g. `if(tempEnt.NPC)`) saw it as present -> the GNPC/PARM sub-chunk would
		// be written but not read -> stream desync (same class as the GCLI -2 sentinel bug). `*(void**)pv`
		// is 4 bytes on ILP32 (identical to before) and 8 bytes on LP64 (correct). The gate fields here are
		// gentity_t.NPC and gentity_t.parms.
		*(qboolean *)pv = !!(*(void **)pv);
		break;
...
#ifdef __LP64__
// ILP32<->LP64 savegame transcoder (g_savetranscode.cpp). On 64-bit builds the raw struct-dump chunks
// GENT/GCLI/GNPC/LVLC are (de)serialized in the canonical 32-bit on-disk layout, so saves stay
// byte-identical to the armv7/PC-retail format. Returns NULL for width-stable chids (raw passthrough).
extern const void* SG_TranscodeDesc(unsigned long chid);
extern int  SG_TranscodePack(const void* desc, const unsigned char* native, unsigned char* disk);
extern void SG_TranscodeUnpack(const void* desc, const unsigned char* disk, unsigned char* native);
extern int  SG_TranscodeDiskSize(const void* desc);
#endif
...
#ifdef __LP64__
	{
		const void *sgd = SG_TranscodeDesc(ulChid);
		if ( sgd ) {
			unsigned char sgdisk[8192];   // max transcoded chunk is GCLI = 3364 bytes
			int ds = SG_TranscodePack(sgd, (const unsigned char *)pbData, sgdisk);
			gi.AppendToSaveGame(ulChid, sgdisk, ds);   // emit the canonical ILP32 blob, not the wider LP64 struct
		} else {
...
*/
	...
	case F_ALERTEVENT:
		{				
			...
			for (int i=0; i<MAX_ALERT_EVENTS; i++)
			{
				p[i].owner = GetGEntityPtr((int)(intptr_t)(p[i].owner));	// saved slot holds the entity index in its low 32 bits (LP64-safe)
			}
		}
		break;
...
//	}
	...
	int iReadSize;
	{
		const void *sgd = SG_TranscodeDesc(ulChid);
		if ( sgd ) {
			unsigned char sgdisk[8192];
			int ds = SG_TranscodeDiskSize(sgd);
			int got = gi.ReadFromSaveGame(ulChid, sgdisk, bOkToSizeMisMatch?0:ds, NULL);
			if ( got > 0 ) {
				SG_TranscodeUnpack(sgd, sgdisk, (unsigned char *)pbData);
				iReadSize = iSize;   // report the native size so the (iReadSize!=iSize) check below passes
			} else {
				iReadSize = 0;
			}
		} else {
			iReadSize = gi.ReadFromSaveGame(ulChid, pbData, bOkToSizeMisMatch?0:iSize, NULL);
		}
	}
#else
...
void WriteGEntities(qboolean qbAutosave)
{
	...
	for (int i=0; i<(qbAutosave?1:globals.num_entities); i++)
	{
		...
		if ( ent->inuse)
		{
			...
			// EF1 SP arm64 (LP64) FIX: EnumerateField marks an NPC client by writing the -2 sentinel as
			// `*(int *)pv = -2` (case F_GCLIENT) -- which sets ONLY the low 32 bits of the 8-byte `client`
			// pointer, leaving the high 32 bits = the original pointer. So the full-pointer compare
			// `tempEnt.client == (gclient_t *)-2` (= 0xFFFFFFFFFFFFFFFE) is FALSE on arm64 -> the GCLI
			// sub-chunk was NEVER written for NPC clients. But ReadGEntities restores -2 via GetGClientPtr
			// (a properly sign-extended (gclient_t *)-2) and so its `pEnt->client == (gclient_t *)-2` IS
			// true and tries to read GCLI -> the write/read conditions disagree -> the savegame stream
			// desyncs ('non-monotonic entity index ... skipping gap-free'), entities downstream lose their
			// ICARUS sequencer (scriptrunner sequencer==NULL -> scripts abort -> e.g. borg1 strut explosion
			// leaves the player clipped in solid). Compare the low-32 marker slot that EnumerateField
			// actually wrote: ABI-correct on BOTH (ILP32: the whole 4-byte field == -2; LP64: the written
			// low slot == -2). Worked on retail/armv7 (ILP32) only because `client` was 4 bytes there.
			// NOTE: only saves WRITTEN AFTER this fix are clean; pre-fix arm64 saves are missing the GCLI
			// chunks and remain desynced -- start from a fresh save. See memory `arm64-gcli-sentinel-desync`.
			if ( *(int *)&tempEnt.client == -2 )
			{
...
void ReadGEntities(qboolean qbAutosave)
{
	...
	for (int i=0; i<iCount; i++)
	{
		...
		// A non-monotonic saved entity index (iEntIndex <= iPreviousEntRead) means the chunk stream
		// desynced earlier (e.g. an old-format / different-layout save where a per-entity conditional or
		// string chunk count didn't match) OR a genuinely out-of-order save. Warn so it's visible in logs
		// instead of silently corrupting; the bounded gap-free below then degrades gracefully (no crash).
		if ( iEntIndex <= iPreviousEntRead )
			Com_Printf("savegame: non-monotonic entity index %d after %d (stream desync / old-format save) -- skipping gap-free\n", iEntIndex, iPreviousEntRead);

		// Retail freed the entities in the (prev, iEntIndex) gap with `for(j=prev+1; j!=iEntIndex; j++)`,
		// which ASSUMES strictly-increasing saved indices. If a read yields iEntIndex <= iPreviousEntRead
		// (non-monotonic), j starts above iEntIndex and increments away from it forever -> it runs off the
		// end of g_entities[MAX_GENTITIES] into garbage, then G_FreeEntity/ICARUS_FreeEnt derefs a garbage
		// script_targetname -> SIGSEGV (the cross-ABI holodeck-load crash). Bound the loop: only free a
		// genuine forward gap, and never index past MAX_GENTITIES.
		if ( iEntIndex > iPreviousEntRead+1 )
		{
			for (int j=iPreviousEntRead+1; j < iEntIndex && j < MAX_GENTITIES; j++)
			{
