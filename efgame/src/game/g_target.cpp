...
void target_teleporter_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	TeleportPlayer( activator, dest->s.origin, dest->s.angles, STARFLEET_TELEPORT_FX );
}
...
{
	// SP port — spawn warm-up gate. On Android the bridge fires the opening scriptrunners at
	// level.time~0 (during SP_StartClient's first RunFrame, before CG_INIT), but late-spawning
	// NPC/cinematic entities (tuvok, MUNROSTARTCIN, ...) only register in ICARUS_EntList when
	// their NPC_Begin think runs (~level.time 100-200, after CG_INIT). Retail's fixed-50ms
	// SV_Frame cadence keeps these ordered; our variable-delta bridge does not, so affect()
	// targets resolve NULL and the whole dialog/cinematic block is silently discarded
	// (CSequencer::ParseAffect). Defer the first run past a warm-up so those entities exist.
	// Re-fires via think at 1000 (>= so it never re-defers); preserves self->activator.
	if ( level.time < 1000 )
	{
...
void target_level_change_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if ( Q_strncmp( "_holo", self->message, 5 ) == 0 )
	{//We're going to load a holodeck map, save the exitholodeck savegame first
		if ( Q_strncmp( "_holo", level.mapname, 5 ) != 0 )
		{//but only if we're not already on a holodeck map
			if ( g_virtualVoyager->value == 1 )
			{//in tour mode, stomp the virtual voyager progess save
				gi.SendConsoleCommand( "save virtual\n" );
			}
			else
			{
				gi.SendConsoleCommand( "save exitholodeck\n" );
			}
		}
	}
...
void target_interface_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{	 
	gi.SendConsoleCommand( va("genericmenu %s\n",self->script_targetname));
}
...
void SP_target_interface( gentity_t *self ) 
{
	...
	self->e_UseFunc = useF_target_interface_use;
