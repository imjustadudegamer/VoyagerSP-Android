...
void G_SpawnEntitiesFromString( const char *entityString ) {
	...
	{//World has a spawn script, but we don't want the world in ICARUS and running scripts,
		...
		if ( script_runner )
		{
			...
			// SP port: delay the worldspawn master script's first think so spawn-time NPC/cin
			// entities (NPC_Begin think @ level.time+100..200) are registered in ICARUS_EntList
			// BEFORE this script Runs and resolves its affect() targets. Retail's fixed-50ms
			// SV_Frame cadence masks this race; our bridge advances by variable wall-clock delta.
			// (FIX B.1 — covers level-load entities; mid-level USE-triggered targets need FIX B.4.)
			script_runner->nextthink = level.time + 500;
