...
void NPC_Begin (gentity_t *ent)
{
	...
	{//No NPCs should telefrag
		if( SpotWouldTelefrag( ent, TEAM_FREE ) )//(team_t)(ent->client->playerTeam)
		{
			...
			{
				Q3_DebugPrint( WL_DEBUG, "NPC %s could not spawn, waiting %4.2f secs to try again\n", ent->targetname, ent->wait/1000.0f );
...
//	ent->s.origin[2] += 1;	// make sure off ground
	...
	//ICARUS include
	if ( ent->sequencer == NULL )   // SP: NPC may already be ICARUS-registered at spawn (NPC_Spawn_Go)
		ICARUS_InitEnt( ent );
...
//=====================================================================
	...
	// SP port: register this NPC in ICARUS_EntList NOW (at spawn) instead of one frame later in
	// NPC_Begin. ICARUS resolves affect("npcname") EAGERLY at script-parse time, so a name not yet
	// registered (deferred NPC_Begin think) gets its whole affect block silently discarded — the
	// level-start dialog/cinematic race. newent is freshly G_Spawn'd (sequencer/taskManager NULL =
	// ICARUS_InitEnt precondition); cleanup is handled by G_FreeEntity -> ICARUS_FreeEnt.
	if ( ICARUS_ValidEnt( newent ) )
...
void SP_NPC_Species8472( gentity_t *self)
{
	...
	if (g_language && Q_stricmp("DEUTSCH",g_language->string)==0)
	{
		self->fullName = "Fl�ssigraum-Lebensform";
	}
	else
	{
