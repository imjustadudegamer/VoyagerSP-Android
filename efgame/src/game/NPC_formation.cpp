...
#define SQUAD_AVOID_COLL 0

//=========================================================================================
#ifdef SQUAD_AVOID_COLL
void NAVF_ClearBlockedInfo (gentity_t *self)
{
	self->NPC->aiFlags &= ~NPCAI_BLOCKED;
	self->NPC->blockingEntNum = ENTITYNUM_WORLD;
}

void NAVF_SetBlockedInfo (gentity_t *self, int entId)
{
	self->NPC->aiFlags |= NPCAI_BLOCKED;
...
/*
qboolean NAV_CheckAvoidCollision(gentity_t *self, const vec3_t destOrg, vec3_t pdir, float maxCheckDist, qboolean setBlockedInfo, int okayToHitEnt)

Only remaining trace bug I know of:
	They seem to think they can move right through each other- traces come back with
	a fraction of 1.0 even though they're clearly blocked by another NPC...

  Maybe for now I can do a line trace from the origin???

  Also, sometimes there is still a problem going up steps, but that may lie in
  PM_StepSlideMove
*/
qboolean NAVF_CheckAvoidCollision(gentity_t *self, const vec3_t destOrg, vec3_t pdir, float maxCheckDist, qboolean setBlockedInfo, int okayToHitEnt)
{
	trace_t	trace;//, trace2;
	vec3_t	dest, dest2, dest3, mins, maxs, up, right, vec, dir, avoidAngles, forward;
	float	avoidRadius;
	float	checkDist;
	float	straight_fraction = 0.0f, right_fraction = 0.0f, left_fraction = 0.0f;
	int		blocker = -1;
	vec3_t	diffVec;
	float	dist=0, arcAngle, yaw;//, circumference;
	float	dot = 0.0f;

	//FIXME: still need to know if blocked?
	if(self->NPC->aiFlags & NPCAI_NO_COLL_AVOID)
	{//We don't care about bumping into stuff
		return qtrue;
	}

	VectorSet(up, 0, 0, 1);
	
	//dir MUST be normalized!  This is costly, I suppose, but must be done
	VectorCopy(pdir, dir);
	VectorNormalize(dir);
	...
	if(!maxCheckDist || maxCheckDist > MAX_COLL_AVOID_DIST)
	{
		maxCheckDist = MAX_COLL_AVOID_DIST;
	}

	VectorSubtract(destOrg, self->currentOrigin, vec);
	checkDist = VectorLength(vec);

	if(checkDist > maxCheckDist)
	{
		checkDist = maxCheckDist;
	}

	VectorMA(self->currentOrigin, checkDist, dir, dest);
	gi.trace(&trace, self->currentOrigin, mins, maxs, dest, self->s.number, self->clipmask);
	...
	{//Got there going straight!
		/*//Think I fixed this!
		gi.trace(&trace, self->currentOrigin, NULL, NULL, dest, self->s.number, self->clipmask);\
		if(trace.fraction != 1.0)
		{//If this asserts, something is fucked up with traces, this is the same trace
			//as the previous one, just with no size!
			//assert(0);
			Debug_Printf(debugNPCAI, DEBUG_LEVEL_ERROR, "ERROR: Full trace failed, fallback line trace succeeded\n");
		}
		else*/
		{
			return qtrue;
	...
	{
		if ( okayToHitEnt >= 0 && okayToHitEnt < ENTITYNUM_WORLD )
		{
			gentity_t *checkEnt = &g_entities[okayToHitEnt];
			if ( checkEnt && checkEnt->svFlags & SVF_NAVGOAL && self->NPC && self->NPC->goalEntity == checkEnt )
			{//Going after a navgoal
				if ( NAV_HitNavGoal( trace.endpos, self->mins, self->maxs, checkEnt->currentOrigin, checkEnt->maxs[0] ) )
				{
					return qtrue;
				}
			}
		...
		if ( trace.fraction * checkDist > 24 )
		{//Still at least 24 from the blocker
			gentity_t *obstacle;
			obstacle = &g_entities[trace.entityNum];
			if ( obstacle )
			{
				if ( Q_stricmp("func_door", obstacle->classname) == 0 )
				{//Okay to proceed, it should open if we get closer
					return qtrue;
				}
			}
		}
	...
	if(okayToHitEnt > -1 && okayToHitEnt < ENTITYNUM_WORLD)
	{
		if(trace.entityNum == okayToHitEnt)
		{//Bumped into someone, but it's okay
			return qtrue;
		}
	}

	blocker = trace.entityNum;

	/*
	if(self->NPC->consecutiveBlockedMoves > MAX_NUM_CONSECUTIVE_BLOCKED_MOVES)
	{//stop trying to go around until you can go forward for at least one move?
		//goto failed;
	}
	*/

	/*
	if(blocker == ENTITYNUM_WORLD)		
	{//See if it's just a second step
		mins[2] += STEPSIZE;
		gi.trace(&trace2, self->currentOrigin, mins, maxs, dest, self->s.number, self->clipmask);
		if((trace2.fraction*checkDist) - (trace.fraction*checkDist) >= 8)
		{//The obstacle is 2 step sizes tall and second step is at least 8 deep... it's probably a step, allow it
			return qtrue;
		}
		mins[2] -= STEPSIZE;
	}
	*/
	...
	straight_fraction = trace.fraction;
	//Else Bumped into something, try going to the right
	if(blocker == ENTITYNUM_WORLD)		
	{//Bumped into architecture, try going to right - if we knew how blocked we were, we could determine how far to move to right rather than just half our size...
		VectorCopy(trace.endpos, diffVec);
		avoidRadius = sqrt((self->maxs[0]*self->maxs[0]) + (self->maxs[1]*self->maxs[1]));
		//The closer you are, the further you have to move to side
		avoidRadius += avoidRadius * (1 - trace.fraction);
		//FIXME: take into account trace plane's normal too?

		VectorMA(diffVec, avoidRadius, right, dest2);
		VectorSubtract(dest2, self->currentOrigin, forward);
		VectorNormalize(forward);
		VectorMA(self->currentOrigin, checkDist, forward, dest2);
	}
	else
	{
		VectorSubtract(g_entities[blocker].currentOrigin, self->currentOrigin, diffVec);
		dist = VectorNormalize(diffVec);
		yaw = vectoyaw(diffVec);
		//CrossProduct(diffVec, up, diffRight);
		avoidRadius = sqrt((g_entities[blocker].maxs[0]*g_entities[blocker].maxs[0]) + (g_entities[blocker].maxs[1]*g_entities[blocker].maxs[1])) + sqrt((self->maxs[0]*self->maxs[0]) + (self->maxs[1]*self->maxs[1]));
		if(dist <= avoidRadius)
		{//Inside each other!
			arcAngle = 135;
		}
		...
		{
			//FIXME: Using dist, need to increase the avoidRadius the lower dist is!!!
			//use a formula to find a point on circumference of
			//circle given arclength, radius and one point of the arc.
			arcAngle = avoidRadius / dist * 60 + 4.0;//Add 4 degrees just for the hell of it
		}

		/*
		circumference = 2*M_PI*dist;
		arcAngle = avoidRadius / circumference * 360;
		*/
		if(arcAngle > 180)
		{//WTF?
			assert(0);
		...
		//default arcAngle will go left
		dot = DotProduct(diffVec, right);
		if(dot < 0.0f)
		{//The obstacle is more to the left, so try to the right first
			arcAngle *= -1;
		}

		VectorClear(avoidAngles);
		avoidAngles[YAW] = AngleMod(yaw + arcAngle);
		AngleVectors(avoidAngles, forward, NULL, NULL);
		VectorMA(self->currentOrigin, dist, forward, dest2);
	}

	//FIXME: avoidRadius should be larger if close to obstacle???
//		avoidRadius *= 1.25;//Give it a little berth

	//Try going to right
	gi.trace(&trace, self->currentOrigin, mins, maxs, dest2, self->s.number, self->clipmask);
	if(trace.allsolid || trace.startsolid)
	{//Would have happened above if at all, but just to be safe
		goto failed;
	}

	if(trace.fraction >= 1.0)
	{//Clear to the right!
		VectorSubtract(dest2, self->currentOrigin, pdir);
		if(blocker != ENTITYNUM_WORLD)
		{//Extend to correct length again
			VectorNormalize(pdir);
			VectorScale(pdir, checkDist, pdir);
		}
		return qtrue;
	}
	
	//Else Bumped into something going right, so try left
	if(blocker == ENTITYNUM_WORLD)		
	{//Bumped into architecture, try going to left - if we knew how blocked we were, we could determine how far to move to right rather than just half our size...
		right_fraction = trace.fraction;
		//FIXME: take into account trace plane's normal too?
		VectorMA(diffVec, -avoidRadius, right, dest3);
		VectorSubtract(dest3, self->currentOrigin, forward);
		VectorNormalize(forward);
		VectorMA(self->currentOrigin, checkDist, forward, dest3);
	}
	else
	{
		//TEMP: For now, try only one side?
		//Yes: otherwise, ping pong!
		if(self->NPC->canShove)
		{//FIXME: if do this too many times, just stand there
			gentity_t	*pushEnt = NULL;
			float		pushStrength = (checkDist - dist);
			float		shoveDir = dot;

			if(blocker < ENTITYNUM_WORLD && g_entities[blocker].client && g_entities[blocker].NPC && g_entities[blocker].client->playerTeam == self->client->playerTeam )
			{//Shove NPCs out of the way
				 pushEnt = &g_entities[blocker];
			}
			else
			{//Shove self out of the way
				if(self->client->race != RACE_BORG)
				{//Borg don't sidestep
					pushEnt = self;
					pushStrength *= -1;
				}
			...
			if(pushEnt != NULL)
			{
				...
				else if(pushStrength > 300)
				{
					pushStrength = 300;
				}

				//FIXME: This could ping-pong, need a debounce time...
				
				if(self->NPC->blockingEntNum < ENTITYNUM_WORLD && self->NPC->blockingEntNum != blocker )
				{//We're pushing someone other than the last guy we shoved
					self->NPC->shoveDebounce = 0;
				}

				if(self->NPC->shoveDebounce > level.time)
				{//We were already shoving, so 
					shoveDir = self->NPC->lastShoveDir;
				}
				else
				{
					self->NPC->shoveDebounce = level.time + 1000;
					self->NPC->lastShoveDir = dot;
					self->NPC->shoveCount++;
				}
	#ifdef _DEBUG
				//Debug_Printf(debugNPCAI, DEBUG_LEVEL_INFO, "%s shoving %s ", self->targetname, pushEnt->targetname);
	#endif
				if(shoveDir < 0.0f)
				{//The obstacle is more to the left, so shove them that way
	#ifdef _DEBUG
					//Debug_Printf(debugNPCAI, DEBUG_LEVEL_INFO, "left %4.2f \n", pushStrength);
	#endif
					VectorMA(pushEnt->s.pushVec, pushStrength * -1, right, pushEnt->s.pushVec);
				}
				else
				{
	#ifdef _DEBUG
					//Debug_Printf(debugNPCAI, DEBUG_LEVEL_INFO, "right %4.2f \n", pushStrength);
	#endif
					VectorMA(pushEnt->s.pushVec, pushStrength, right, pushEnt->s.pushVec);
				}
			}
		}
		goto failed;
		...
		avoidAngles[YAW] = AngleMod(yaw - arcAngle);
		AngleVectors(avoidAngles, forward, NULL, NULL);
		VectorMA(self->currentOrigin, dist, forward, dest3);*/
	}

	gi.trace(&trace, self->currentOrigin, mins, maxs, dest3, self->s.number, self->clipmask);
	if(trace.allsolid || trace.startsolid)
	{//Would have happened above if at all, but just to be safe
		goto failed;
	}
	...
	if(trace.fraction >= 1.0)
	{//Clear to the left!
		VectorSubtract(dest3, self->currentOrigin, pdir);
		if(blocker != ENTITYNUM_WORLD)
		{//Extend to correct length again
			VectorNormalize(pdir);
			VectorScale(pdir, checkDist, pdir);
	...
	if(blocker == ENTITYNUM_WORLD)
	{//If either try to the side went farther than straight, pick farthest of two
		//FIXME: will this ping-pong?  Doesn't seem too...
		left_fraction = trace.fraction;

		if(left_fraction > straight_fraction)
		{
		...
		else if(right_fraction > straight_fraction)
		{//Go right
			VectorSubtract(dest2, self->currentOrigin, pdir);
			return qtrue;
		}
...
failed:
	if(setBlockedInfo)
	{
		NAVF_SetBlockedInfo (self, blocker);
	}
	return qfalse;
}

void NPC_CheckBlockedState( gentity_t *self )
{
		...
		else
		{
			...
			{//Should we ask it to get out of the way?
				...
				if(NPCInfo->blockedSpeechDebounceTime < level.time)
				{
					...
					NPCInfo->blockedSpeechDebounceTime = level.time + 3000;//FIXME: make a define
	...
	NAVF_ClearBlockedInfo(NPC);
}
#endif//SQUAD_AVOID_COLL
//=========================================================================================
/*
-------------------------
NPC_SquadIdle
...
void NPC_Unobstruct( vec3_t dir )
{
	...
	for ( int i = 0; i < AVOID_ITERATIONS; i++ )
	{
		...
		if ( NAV_CheckAhead( NPC, testPos, tr, NPC->clipmask ) )
		{
...
*/
	...
	gi.trace( &trace, goalOrg, self->mins, self->maxs, bottom, self->s.number, CONTENTS_SOLID|CONTENTS_MONSTERCLIP );
...
#ifdef SQUAD_AVOID_COLL
					if ( !NAVF_CheckAvoidCollision( NPC, goalPos, dir, 128, qtrue, -1 ) )
#else
			...
			{//Waiting around at sPDestPos
				...
				{//FIXME: need to remember last lookMode
					...
					if ( ( changelook < 20 || torsoAnim == BOTH_STAND2TO4 || torsoAnim == BOTH_STAND4 || torsoAnim == BOTH_STAND4TO2 ) && 
						NPC->client->ps.weapon != WP_NONE && NPC->client->ps.weapon != WP_TRICORDER && NPC->client->ps.weapon != WP_PHASER && 
					...
					{//Play one of the lookarounds instead
						...
						NPCInfo->desiredYaw = AngleMod(pathAngles[YAW]);
						NPCInfo->desiredPitch = AngleMod(pathAngles[PITCH]);
					...
					{//FIXME: do this also when walking around?
						...
						{//FIXME: this turning should be torso only...
							NPCInfo->desiredYaw = AngleMod( pathAngles[YAW] + Q_irand( 10, 30 ) );
						...
						if ( lookMode != LT_NONE )
						{
							...
							if ( changelook > 90 )
							{//FIXME: this turning should be torso only...
								NPCInfo->desiredPitch = AngleMod( pathAngles[PITCH] + Q_irand( 1, 20 ) );
								NPC->aimDebounceTime = level.time + Q_irand( 500, 2000 );
							}
							else if ( changelook > 80 )
							{
