...
extern vmCvar_t cg_thirdPerson;
		...
		if ( player_locked && ent->client->ps.pm_type < PM_DEAD ) {//lock out player MOVEMENT (not look)
			// EFSP: do NOT clear ucmd->angles here. SET_PLAYER_LOCKED means "player cannot MOVE"
			// (Q3_Interface SET_PLAYER_LOCKED help text); retail leaves the VIEW free so the player can
			// still look/aim during a locked conversation (e.g. deck02/telstalk: SET_PLAYER_LOCKED true +
			// SET_LOOK_TARGET seven — device/retail-confirmed you can look around). Clearing the angles
			// here also froze the SERVER view while the client prediction kept the raw input angles, so
			// the snapshot's frozen angle bled through once per 50ms snapshot -> a rapid "epileptic" view
			// flicker (the view angle alternated). Locking movement only fixes both: free look + no
			// desync. (SET_LOOK_TARGET still aims the head model; full cutscenes use in_camera instead.)
			ucmd->forwardmove = 0;
			ucmd->rightmove = 0;
			ucmd->buttons = 0;
			ucmd->upmove = 0;
