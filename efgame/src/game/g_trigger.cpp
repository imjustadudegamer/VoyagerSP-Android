...
{
	// EFSP re-entrancy guard (trigger-scoped, 1:1-safe): an end-of-level
	// trigger_multiple re-fires its BSET_USE usescript every think while the
	// player stands locked on it; restarting an already-running script
	// re-triggers its view effect -> the "immediately before transition"
	// flicker. Skip ONLY the re-activation while this ent's own sequencer is
	// still running. Retail is visually idempotent here; this matches that
	// without a blanket ICARUS guard (which would break target_scriptrunner
	// count/wait repeats + affect FLUSH). nextthink + G_UseTargets still fire.
	if ( !( ent->behaviorSet[BSET_USE] && ent->sequencer && ent->sequencer->IsRunning() ) )
	{
