...
int CTaskManager::GetVector( int entID, CBlock *block, int &memberNum, vector_t &value )
{
	...
	if ( Check( ID_TAG, block, memberNum ) )
	{
		...
		memberNum++;
		VALIDATE ( Get( entID, block, memberNum, &tagName ) );
		VALIDATE ( GetFloat( entID, block, memberNum, tagLookup ) );
...
int CTaskManager::Wait( CTask *task, bool &completed  )
{
	...
	{
		VALIDATE( GetFloat( m_ownerID, block, memberNum, dwtime ) );
...
int CTaskManager::Rotate( CTask *task )
{
	...
	if ( Check( ID_TAG, block, memberNum ) )
	{
		...
		VALIDATE( Get( m_ownerID, block, memberNum, &tagName ) );
		VALIDATE( GetFloat( m_ownerID, block, memberNum, tagLookup ) );
	...
	{
		//Get a normal vector
		VALIDATE( GetVector( m_ownerID, block, memberNum, vector ) );
	}

	//Find the duration
	VALIDATE( GetFloat( m_ownerID, block, memberNum, duration ) );
...
int CTaskManager::Camera( CTask *task )
{
	...
	case TYPE_PAN:
		...
		VALIDATE( GetVector( m_ownerID, block, memberNum, vector ) );
		VALIDATE( GetVector( m_ownerID, block, memberNum, vector2 ) );

		VALIDATE( GetFloat( m_ownerID, block, memberNum, fVal ) );
	...
	case TYPE_ZOOM:
		...
		VALIDATE( GetFloat( m_ownerID, block, memberNum, fVal ) );
		VALIDATE( GetFloat( m_ownerID, block, memberNum, fVal2 ) );
	...
	case TYPE_FADE:
		...
		VALIDATE( GetFloat( m_ownerID, block, memberNum, fVal3 ) );
...
int CTaskManager::Move( CTask *task )
{
	...
	if ( GetVector( m_ownerID, block, memberNum, vector2 ) == false )
	{
		...
		(m_owner->GetInterface())->I_DPrintf( WL_DEBUG, "%4d move( <%f %f %f>, %d ); [%d]", m_ownerID, vector[0], vector[1], vector[2], duration, task->GetTimeStamp() );
	...
	(m_owner->GetInterface())->I_DPrintf( WL_DEBUG, "%4d move( <%f %f %f>, <%f %f %f>, %d ); [%d]", m_ownerID, vector[0], vector[1], vector[2], vector2[0], vector2[1], vector2[2], duration, task->GetTimeStamp() );
...
void CTaskManager::Load( void )
{
	...
	CBlock			*block;
	CTask			*task;
	unsigned int	timeStamp;	// TKTS is fixed 4 bytes in retail; pin to 32-bit (DWORD is 8 bytes on arm64). See Save() above.
	bool			completed;
	void			*bData;
	...
	for ( int i = 0; i < numTasks; i++ )
	{
		...
		for ( int j = 0; j < numMembers; j++ )
		{
			...
			case ID_RANDOM:
				block->Write( ID_RANDOM, ID_RANDOM );
				break;
