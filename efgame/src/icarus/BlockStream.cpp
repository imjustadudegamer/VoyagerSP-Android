...
int CBlockMember::ReadMember( char **stream, long *streamPos )
{
	...
	*streamPos += sizeof( int );
	...
	// .IBI stores the member size as a 4-byte field (int m_size, authored on 32-bit). Reading it as
	// `long` grabbed 8 bytes on LP64 (arm64) -> garbage size -> malloc/memcpy crash at spawn. Read 32-bit.
