...
EFSP: true while this sequencer still has queued commands OR its task manager
has pending tasks. Used by multi_trigger_run (g_trigger.cpp) to avoid
restarting an already-running BSET_USE script every think (pre-transition
flicker). Trigger-scoped only — does not affect scriptrunner count/wait repeats.
-------------------------
*/

bool CSequencer::IsRunning( void )
{
	return ( m_numCommands > 0 ) || ( m_taskManager && m_taskManager->IsRunning() );
}
...
{
	CSequencer	*stream_sequencer;
	char		*entname;
	int			ret;

	entname = (char*) block->GetMemberData( 0 );

	stream_sequencer = m_ie->I_GetEntityByName( entname );
	...
	//FIXME: If the target sequence is freed, what then?
...
void CSequencer::CheckAffect( CBlock **command )
{
	...
	{
		CSequencer *sequencer	= m_ie->I_GetEntityByName( (char*) block->GetMemberData( 0 ) );
		int			type		= (int) (*(float *) block->GetMemberData( 1 ));
		int			id			= (int) (*(float *) block->GetMemberData( 2 ));
...
int	CSequencer::Save( void )
{
	...
	m_ie->I_WriteSaveData( 'SQCT', &curGroupID, sizeof ( curGroupID ) );	// was sizeof(m_numCommands) (both int=4, but wrong variable)
